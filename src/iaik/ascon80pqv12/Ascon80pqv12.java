...
public class Ascon80pqv12 {

  // Defines
  public final static int CRYPTO_KEYBYTES = 20;
  ...
  static int crypto_aead_decrypt(byte m[], int mlen, byte nsec[], byte c[], int clen, byte ad[],
    ...
    int b = 6;
    int s = adlen / rate + 1;
    int t = (clen - tlen) / rate + 1;
    int l = (clen - tlen) % rate;
    ...
    boolean error = false;
    for (i = 0; i < tlen; ++i)
      if (c[clen - tlen + i] != S[size - tlen + i])
        error = true;
    if (error)
        return -1;
